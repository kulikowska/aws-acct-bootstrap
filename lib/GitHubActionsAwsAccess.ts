...
export default class GitHubActionsAwsAccess extends Construct {
  ...
  constructor(scope: Construct, id: string, props: GitHubActionsAwsAccessProps) {
    super(scope, id);
    const { user, repo, branchRestrictions } = props.repoConfig;

    const maxSessionDuration = props.maximumSessionDurationInSeconds;
    if (
      (isNaN(maxSessionDuration) ||
        maxSessionDuration > ceilingStsSessionDuration) ||
      maxSessionDuration < floorStsSessionDuration
    )
      throw new Error(
        'STS Session Duration must be between 900 and 43200 seconds'
      );
    const githubIdp = new OpenIdConnectProvider(this, 'GithubIdp', {
    ...
    const githubActionsAwsAccessRole = new Role(this, 'GitHubActionsAwsAccessRole', {
      assumedBy: new WebIdentityPrincipal(
        githubIdp.openIdConnectProviderArn,
        accessConditions
      ),
      maxSessionDuration: Duration.seconds(maxSessionDuration),
    });
    ...
    githubActionsAwsAccessRole.attachInlinePolicy(
      new Policy(this, 'GithubAccessPolicy', {
        statements: [
          new PolicyStatement({
            effect: Effect.ALLOW,
            actions: props.allowedIamActionsForGitHubActions,
